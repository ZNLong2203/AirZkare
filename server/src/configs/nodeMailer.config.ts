        ...
        const mailOptions = {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
                    ...
                    <p><strong>Flight Number:</strong> ${session.flight_details.flight_number}</p>
                    <p><strong>Departure:</strong> ${session.flight_details.departure}</p>
                    <p><strong>Destination:</strong> ${session.flight_details.destination}</p>
                    <p><strong>Date:</strong> ${session.flight_details.date}</p>
                    <p><strong>Time:</strong> ${session.flight_details.time}</p>
                    
                    <div style="border-top: 1px solid #ddd; margin: 20px 0;"></div>

                    <h3 style="color: #333;">Payment Information</h3>
                    <p><strong>Order Number:</strong> ${session.id}</p>
                    <p><strong>Total Amount Paid:</strong> $${(session.amount_total / 100).toFixed(2)}</p>
